...
impl<'docker> Service<'docker> {
    ...
    pub  fn delete(&self) -> Result<Request, Error> {
        self.docker
            .delete(&format!("/services/{}", self.name))
    }
...
pub struct ServiceOptionsBuilder {
    auth: Option<RegistryAuth>,
    params: HashMap<&'static str, crate::errors::Result<Value>>,
}
...
impl ServiceOptionsBuilder {
    ...
    pub fn build(&mut self) -> crate::errors::Result<ServiceOptions> {
...
fn to_json_value<T>(value: T) -> crate::errors::Result<Value>
where
    T: Serialize,
{
